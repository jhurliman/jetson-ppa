...
EIGEN_VERSION=3.4.0
DEB_FILENAME="libeigen3-dev_${EIGEN_VERSION}_arm64.deb"
BUILD_DIR="${SCRIPT_DIR}/../build/eigen-jetson-r32"
META_DIR="${SCRIPT_DIR}/../meta"

if [ ! -f ${BUILD_DIR}/${DEB_FILENAME} ]; then
  echo "${BUILD_DIR}/${DEB_FILENAME} not found. Run ./scripts/build-eigen-jetson-r32.sh first."
  exit 1
fi
